...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div ref={ref} className="relative max-w-4xl mx-auto">
      {/* Timeline line */}
      <motion.div
        className="absolute left-4 md:left-1/2 md:-translate-x-0.5 top-0 bottom-0 w-0.5 bg-gradient-to-b from-blue-500 via-purple-500 to-blue-500"
        initial={{ height: 0 }}
      ...
      <div className="space-y-12">
        {data.map((item, index) => (
          ...
          >
            {/* Timeline dot */}
            <motion.div
              className="absolute left-4 md:left-1/2 md:-translate-x-1/2 w-4 h-4 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full border-4 border-background shadow-lg z-10"
              initial={{ scale: 0 }}
            ...
            <motion.div
              className="absolute left-4 md:left-1/2 md:-translate-x-1/2 w-4 h-4 bg-blue-500 rounded-full opacity-20"
              animate={{ scale: [1, 2, 1] }}
